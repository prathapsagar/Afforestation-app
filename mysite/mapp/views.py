...
def get_image(lat, lon):
    ...
    uri = url+"center=" + center + "&zoom="+zoom+"&scale=" + \
        scale+"&size="+size+"&maptype="+maptype+"&key="+key
    r = requests.get(uri)
    img_file = "hello.png"
    ...
    detections = detector.detectObjectsFromImage(
        input_image="mapp/image/hello.png", output_image_path="mapp/static/mapp/hello2.png", thread_safe=True)
...
class MapCreateView2(View):
    def post(self, request):
        text1 = str(request.POST['latitude'])
        text2 = str(request.POST['longitude'])
        # text = 'Vishwas'
        # t1 = text.split(",")
        # get_image(t1[0], t1[1])
        get_image(text1, text2)
        return render(request, 'mapp/output.html')
